...
suas mãos do jeito que for mais fácil — o link do replay, um print, um arquivo, um
áudio — e recebe em segundos uma análise técnica de verdade: onde você ganhou, onde
deixou dinheiro na mesa e o que treinar para evoluir.
...
**equilíbrio Nash computado**, e os spots pós-flop passam por um **solver de
verdade**. Números certos, conselho certo — e o gráfico de range junto, para você
VER o que o coach está falando.

E ele **lembra de você**: cada mão enviada alimenta seu perfil e deixa o coaching
...
2. Toque em **Iniciar** — já tem uma mão de teste carregada para você treinar
3. Mande uma mão sua — a análise chega em instantes
...
| 📊 **CSV do seu tracker** | Exporte do Hold'em Manager / PokerTracker e anexe. Atenção: o CSV traz o resumo das mãos, não a ação street a street — a análise sai mais enxuta |
| 📑 **PDF** | Relatórios em PDF também funcionam |
| 🎙️ **Áudio** | Grave sua pergunta por voz — o coach transcreve, mostra o que entendeu e responde |
| 🎬 **Vídeo / YouTube** | Mande um vídeo da sessão ou um link do YouTube: o coach extrai os quadros e a transcrição para você apontar a mão. É o passo zero — a análise vem quando você mandar a mão |

💡 **Se vier uma imagem junto com o link, ignora.** O app do clube manda a arte
de propaganda da mesa (vazia) antes do link. A mão está no *link*. O KKNuths
reconhece essa arte e não gasta sua cota com ela.
...
palavra **"lobby"** ou **"estrutura"** na legenda. Sem a palavra, o coach tenta
ler a foto como uma mão — e não vai achar cartas nela.
...
- **Selo e placar street a street** — ✅ jogou bem · 🟡 dava pra jogar melhor · ❌
  erro claro. A primeira linha é o veredito; cada street vem com o número que o
  sustenta (*"pedia 22,9%, você tinha 41%"*)
- **Leitura técnica em português claro** — todo termo técnico vem explicado na
  primeira vez (ex.: *pot odds — o preço que o pote te oferece*)
...
- **🎬 O filme da mão**: cada mão analisada vira um **storyboard** — a mão inteira
  quadro a quadro (pré-flop → river), com o board, os stacks, **o que os vilões
  fizeram em cada street**, a matemática (equity/EV) e o veredito do coach, tudo
  numa imagem só. Bate o olho e entende o spot inteiro
- **Botões depois da análise**: 🎈 *explica mais simples* (a mesma análise em
  linguagem de mesa, sem perder os números) · 🔁 *simular esta mão* · 📖 *range
  do spot* · 📣 *desafiar os amigos* (um card com o spot para o grupo responder)
- **Em torneios**: pressão de ICM (quanto suas fichas valem em dinheiro real),
  ...
  **equilíbrio Nash calculado** — e o equilíbrio certo para a mesa: **mesa cheia
  tem o ante de todo mundo no pote e empurra mais largo que um heads-up**; o coach
  usa o modelo da sua mesa, não um genérico
...
- **O solver diz quão perto chegou.** Quando um spot pós-flop é resolvido, a
  resposta carrega a medida de convergência — se ainda estava grosseira, ela
  avisa e você trata como ordem de grandeza, não como decimal.
...
- **Sem imagem legível, sem veredito.** Foto de mesa vazia ou ilegível não vira
  selo inventado: o coach diz o que faltou e pede o dado certo.

### ⏳ Quando o coach demora

Um spot pós-flop de verdade leva de 10 segundos a pouco mais de um minuto para
resolver — é CFR+ mão a mão, não chute. Você vê **o relógio andando** e a etapa
(*"resolvendo o equilíbrio do flop — 45%"*), e o coach avisa o teto de tempo
antes de começar. Se a mensagem parou de se mexer, aí sim algo deu errado.
...
seu padrão antes de você sentar. Precisa de **sessão inteira** (arquivo ou
texto colado), veja abaixo.

### 📂 Mão avulsa × sessão inteira — o que cada uma libera

Isso importa e o coach é honesto sobre isso:

- **Mão avulsa** (link de replay, print, mão narrada): libera tudo que é sobre
  *aquela* mão — selo, placar, equity, ICM, filme, simulação, range do spot,
  quiz e treino com as suas mãos, perfil do vilão.
- **Sessão inteira** (arquivo .txt, texto colado, CSV): libera também o que
  depende de **frequência** — VPIP, PFR, 3-bet, leaks em bb/100, Tilt Detector,
  estilo, evolução, foco. Frequência calculada só com as mãos que você escolheu
  mandar sairia errada (ninguém manda o replay da mão que largou no pré), então
  o coach **prefere não dar o número a dar o número errado**.

Joga só em clube e não tem arquivo? Manda as mãos que valem a pena — cada uma
rende análise completa, entra no seu treino e no seu quiz.
...
Perguntou *"foi certo o call?"* — a resposta abre com o selo, não com uma aula.
Quer saber **de onde veio um número**, pergunte: ele mostra a conta.
...
- **Estrutura do torneio**: o ritmo (turbo, regular, hyper) **medido nas suas
  mãos**, não adivinhado pelo nome que você digitou. Com o print do lobby
  (legenda "lobby"), ele lê a tabela de blinds, o relógio e o stack inicial.
- **ICM de verdade**: para a conta de dinheiro real o coach precisa da
  premiação — diga *"ficaram 12, paga 9, o 1º leva 1.200"* e ele guarda para o
  torneio inteiro.
...
| `/relatorio` | **O torneio inteiro analisado, mão por mão** (HTML): o filme de cada mão, a análise do coach e o botão 🔍 que reabre qualquer uma no chat. *8+ mãos do mesmo torneio* |
| `/prova` | **Audite a ferramenta** nas suas próprias mãos — classes de verificação, com as falhas na cara |

### 🎮 Treino
| Comando | O que faz |
|---|---|
| `/preparar` | Briefing pré-torneio: seus leaks, protocolo mental e metas. Descreva o torneio junto — `/preparar turbo 5k jogadores, 2h de late reg` — e ele adapta |
| `/simular` | Jogue uma mão SUA de novo, decisão a decisão, com botões — e compare sua linha com a real |
| `/treino` | Drill rápido: o que você faria neste spot? Ao responder, chega o filme da mão. Acertos seguidos viram 🔥 |
| `/leitura` | Adivinhe a mão do vilão a partir da linha que ele tomou |
| `/foco` | No que você está trabalhando agora, e como está indo — um problema por vez, com o critério de alta escrito ANTES. *Precisa de sessão inteira* |

### 📐 Ferramentas
| Comando | O que faz |
|---|---|
| `/spot` | EV de all-in: o equilíbrio do spot, **com e sem ICM** — `/spot open_shove mp 12` · `/spot reshove btn 12 co` |
...
| `/banca` | Risco de ruína e downswing esperado — `/banca 50 15` (50 buy-ins, 15% de ROI) |
...
**Mandei o link e veio uma imagem de mesa vazia antes.**
