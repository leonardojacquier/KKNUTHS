...
## Línea de Minicargadoras (set-2026)

- Fonte: "Catálogo de minicargadoras" (Shandong Hightop Machinery, em espanhol, 22 páginas duplas), enviado pelo dono. As tabelas são imagem: os 20 modelos foram transcritos à mão em `gen-minicargadoras/dados.py`, com a página de origem em cada entrada. O PDF não está no repo.
- Publicado: grupo **Minicargadoras** no bundle (linha + implementos + 20 modelos), `/ventas/minicargadoras/` (categoria), `/ventas/linea-de-minicargadoras/`, `/ventas/implementos-para-minicargadora/`, uma página e uma ficha HTML+PDF por modelo. O card antigo "Minicargadora (Skid Steer)" (marca ZS, 3 colunas) saiu de Movimiento de Suelo; a URL antiga redireciona para a linha.
- Marca nos cards: **HIGHTOP** (o fabricante do catálogo). O card antigo dizia "ZS". Se o dono preferir outra marca, é a constante `MARCA` em `contenido.py`.
- Decisões de transcrição: HYS35 tem "500 kg" sem rótulo na tabela (é a carga operativa, igual ao catálogo ZS); HY-T470 "16bar" de pressão foi omitido por ser valor duvidoso; HYS60 (rodas) e HYS65 (esteira) têm a mesma tabela; a frase "operador de pé na plataforma" só aparece nos modelos em que o catálogo mostra a plataforma.
- Implementos: 95 no catálogo, 89 publicados. Os 6 de neve/gelo ficaram de fora (não se aplicam no Paraguai). Nomes traduzidos para espanhol, com o nome original do catálogo embaixo; o catálogo não traz especificações por implemento.
- Fotos: os recortes com transparência do próprio PDF. HYS45 veio com fundo branco (recortado com rembg/birefnet); HYS382T e HY-V1000 tinham duas máquinas na mesma imagem (fica o maior bloco); HYSL390 tinha o selo "HOT/NEW" (removido). O card da linha usa a foto que já estava no site (`zs-minicargadora.png`).
