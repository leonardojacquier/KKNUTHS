import"./style-DhTN27MS.js";const c={instagram:"https://www.instagram.com/gnhimportacionespy/",facebook:"https://www.facebook.com/edzon.camilomazzonrtto",tiktok:"https://www.tiktok.com/@gnh",whatsapp:"https://wa.me/595985311031?text=Hola,%20quiero%20más%20información"},A=[{ciudad:"Ciudad del Este",dir:"Av. República del Perú km 7, CDE 100101",tel:"0995 360060",share:"https://share.google/OAtbh8akZyeNATtUM",q:"Av. República del Perú km 7, Ciudad del Este, Paraguay"},{ciudad:"Asunción · Ñemby",dir:"Acceso Sur, Ñemby 111210",tel:"",share:"https://share.google/DB7b1OL7m1ISorPhY",q:"Acceso Sur, Ñemby, Paraguay"}];function E(e="footer-slot"){const t=document.getElementById(e);if(!t)return;t.innerHTML=`
      ...
      <div class="mx-auto max-w-6xl px-6 py-12 grid gap-10 md:grid-cols-3 border-t border-white/10 mt-12">
        <div>
          ...
          <div class="mt-5 flex gap-3">
            <a href="${c.instagram}" target="_blank" rel="noopener" aria-label="Instagram" class="foot-social">${L}</a>
            <a href="${c.facebook}" target="_blank" rel="noopener" aria-label="Facebook" class="foot-social">${T}</a>
            <a href="${c.tiktok}" target="_blank" rel="noopener" aria-label="TikTok" class="foot-social">${H}</a>
          </div>
        </div>
        <div>
    ...
    <article class="v-card">
      ...
      <div class="v-card-body">
        ...
        <a class="v-cta" href="${C(t)}" target="_blank" rel="noopener" data-ev="product" data-detail="${e.name}">Consultar ${l("arrow","v-cta-i")}</a>
      </div>
    </article>`}function B(e){return`
    ...
    </article>`}function v(e,t=!1){const a=h.find(s=>s.id===e);if(!a)return;document.querySelectorAll(".v-cat-panel").forEach(s=>s.classList.toggle("is-active",s.dataset.cat===e)),document.querySelectorAll(".v-chip").forEach(s=>s.classList.toggle("is-active",s.dataset.cat===e));let o;a.groups&&a.groups.length?o=a.groups.map(s=>`
      <div class="v-subgroup">
        <h3 class="v-subtitle">${s.title}</h3>
        <div class="v-rail">${s.products.map(w).join("")}</div>
      </div>`).join(""):a.products&&a.products.length?o=`<div class="v-rail">${a.products.map(w).join("")}</div>`:o=`
      <div class="v-empty">
        <p>Pronto sumaremos productos de esta línea.</p>
        <a class="btn-empty" href="${C("Hola, quiero consultar sobre "+a.title)}" target="_blank" rel="noopener">Consultar por WhatsApp</a>
    ...
    ${o}`,r.classList.remove("revealing"),r.offsetWidth,r.classList.add("revealing"),t&&r.scrollIntoView({behavior:"smooth",block:"start"})}function I(){const e=document.getElementById("catalog");e.innerHTML=`
    <div class="v-deck">${h.map(B).join("")}</div>
    <div class="v-products" id="v-products"></div>`,e.querySelectorAll(".v-cat-panel").forEach(a=>{const o=a.dataset.cat;a.addEventListener("click",()=>v(o,!0)),a.addEventListener("keydown",r=>{(r.key==="Enter"||r.key===" ")&&(r.preventDefault(),v(o,!0))})});const t=document.getElementById("cat-chips");t.innerHTML=h.map(a=>`<button class="v-chip" data-cat="${a.id}">${a.title}</button>`).join(""),t.querySelectorAll(".v-chip").forEach(a=>a.addEventListener("click",()=>v(a.dataset.cat,!0))),v(h[0].id)}const M=[];function D(){const e=document.getElementById("promos");if(!M.length){e.style.display="none";return}e.innerHTML=`
    ...
    </div>`}function N(){const e=document.getElementById("quote-form"),t=document.getElementById("quote-status");e.addEventListener("submit",a=>{a.preventDefault();const o=new FormData(e),r=String(o.get("nombre")??"").trim(),s=String(o.get("whatsapp")??"").trim();if(!r||!s){t.textContent="Completá nombre y WhatsApp.",t.className="text-sm text-red-600";return}const m=`Cotización GNH%0ANombre: ${r}%0AEmpresa: ${o.get("empresa")??""}%0AProducto: ${o.get("producto")??""}%0AMensaje: ${o.get("mensaje")??""}`;window.open(`https://wa.me/${k}?text=${m}`,"_blank"),t.textContent="¡Gracias! Te redirigimos a WhatsApp.",t.className="text-sm text-green-600",e.reset()})}z();I();D();N();E();
