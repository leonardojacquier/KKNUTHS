import"./style-B8XEKt7h.js";const c={instagram:"https://www.instagram.com/gnhimportacionespy/",facebook:"https://www.facebook.com/edzon.camilomazzonrtto",tiktok:"https://www.tiktok.com/@gnh",whatsapp:"https://wa.me/595985311031?text=Hola,%20quiero%20más%20información"},q=[{ciudad:"Ciudad del Este",dir:"Av. República del Perú km 7, CDE 100101",tel:"0995 360060",share:"https://share.google/OAtbh8akZyeNATtUM",q:"Av. República del Perú km 7, Ciudad del Este, Paraguay"},{ciudad:"Asunción · Ñemby",dir:"Acceso Sur, Ñemby 111210",tel:"",share:"https://share.google/DB7b1OL7m1ISorPhY",q:"Acceso Sur, Ñemby, Paraguay"}];function S(e="footer-slot"){const r=document.getElementById(e);if(!r)return;r.innerHTML=`
    ...
    </div>`;const a=Array.from(e.querySelectorAll(".vh-slide")),o=Array.from(e.querySelectorAll(".vh-seg")),n=Array.from(e.querySelectorAll(".vh-seg-fill")),s=e.querySelector(".vh-pause"),g=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let d=0,h=g;const $=()=>{a.forEach((i,y)=>i.classList.toggle("is-active",y===d)),o.forEach((i,y)=>{i.classList.remove("is-active","is-done"),y<d&&i.classList.add("is-done")});const t=o[d];t.offsetWidth,t.classList.add("is-active")},b=t=>{d=(t+p.length)%p.length,$()},f=()=>b(d+1),x=()=>b(d-1);n.forEach(t=>t.addEventListener("animationend",()=>{!h&&o[d].classList.contains("is-active")&&f()}));const w=t=>{h=t,e.classList.toggle("is-paused",t),s.setAttribute("aria-pressed",String(t)),s.innerHTML=t?I:k};s.addEventListener("click",()=>w(!h)),e.querySelector(".vh-next").addEventListener("click",f),e.querySelector(".vh-prev").addEventListener("click",x),o.forEach(t=>t.addEventListener("click",()=>b(Number(t.dataset.i)))),e.querySelectorAll(".vh-cta").forEach(t=>t.addEventListener("click",()=>m(t.dataset.cat||"equipos",!0)));const M=e.querySelector(".vh-track");let u=null;M.addEventListener("touchstart",t=>{u=t.touches[0].clientX},{passive:!0}),M.addEventListener("touchend",t=>{if(u!==null){const i=t.changedTouches[0].clientX-u;Math.abs(i)>40&&(i<0?f():x())}u=null},{passive:!0}),$(),g&&w(!0)}function C(e){const r=`Hola, me interesa: ${e.name}${e.brand?" ("+e.brand+")":""}`;return`
    ...
    ${o}`,n.classList.remove("revealing"),n.offsetWidth,n.classList.add("revealing"),r&&n.scrollIntoView({behavior:"smooth",block:"start"})}function N(){const e=document.getElementById("catalog");e.innerHTML=`
    ...
    </div>`}function G(){const e=document.getElementById("quote-form"),r=document.getElementById("quote-status");e.addEventListener("submit",a=>{a.preventDefault();const o=new FormData(e),n=String(o.get("nombre")??"").trim(),s=String(o.get("whatsapp")??"").trim();if(!n||!s){r.textContent="Completá nombre y WhatsApp.",r.className="text-sm text-red-600";return}const g=`Cotización GNH%0ANombre: ${n}%0AEmpresa: ${o.get("empresa")??""}%0AProducto: ${o.get("producto")??""}%0AMensaje: ${o.get("mensaje")??""}`;window.open(`https://wa.me/${E}?text=${g}`,"_blank"),r.textContent="¡Gracias! Te redirigimos a WhatsApp.",r.className="text-sm text-green-600",e.reset()})}j();N();D();G();S();
