import"./style-CsVGQoPs.js";const s={instagram:"https://www.instagram.com/gnhimportacionespy/",facebook:"https://www.facebook.com/edzon.camilomazzonrtto",tiktok:"https://www.tiktok.com/@gnh",whatsapp:"https://wa.me/595985311031?text=Hola,%20quiero%20más%20información"},g=[{ciudad:"Ciudad del Este",dir:"Av. República del Perú km 7, CDE 100101",tel:"0995 360060",share:"https://share.google/OAtbh8akZyeNATtUM",q:"Av. República del Perú km 7, Ciudad del Este, Paraguay"},{ciudad:"Asunción · Ñemby",dir:"Acceso Sur, Ñemby 111210",tel:"",share:"https://share.google/DB7b1OL7m1ISorPhY",q:"Acceso Sur, Ñemby, Paraguay"}];function h(e="footer-slot"){const t=document.getElementById(e);if(!t)return;t.innerHTML=`
      ...
      <div class="mx-auto max-w-6xl px-6 py-12 grid gap-10 md:grid-cols-3 border-t border-white/10 mt-12">
        <div>
          ...
          <div class="mt-5 flex gap-3">
            <a href="${s.instagram}" target="_blank" rel="noopener" aria-label="Instagram" class="foot-social">${v}</a>
            <a href="${s.facebook}" target="_blank" rel="noopener" aria-label="Facebook" class="foot-social">${u}</a>
            <a href="${s.tiktok}" target="_blank" rel="noopener" aria-label="TikTok" class="foot-social">${b}</a>
          </div>
        </div>
        <div>
          ...
          <ul class="mt-4 space-y-2 text-sm">
            ...
            <li><a href="${s.whatsapp}" target="_blank" rel="noopener" class="hover:text-orange">WhatsApp</a></li>
          </ul>
        </div>
        <div>
          <h4 class="font-display text-sm font-bold uppercase tracking-[0.2em] text-white">Seguinos</h4>
          <ul class="mt-4 space-y-2 text-sm">
            <li><a href="${s.instagram}" target="_blank" rel="noopener" class="hover:text-orange">Instagram</a></li>
            <li><a href="${s.facebook}" target="_blank" rel="noopener" class="hover:text-orange">Facebook</a></li>
            <li><a href="${s.tiktok}" target="_blank" rel="noopener" class="hover:text-orange">TikTok</a></li>
          </ul>
        </div>
      </div>
    ...
    <article class="v-card">
      ...
      <div class="v-card-body">
        ...
        <a class="v-cta" href="${x(t)}" target="_blank" rel="noopener" data-ev="product" data-detail="${e.name}">Consultar ${n("arrow","v-cta-i")}</a>
      </div>
    ...
    <article class="v-cat-panel" data-cat="${e.id}" tabindex="0" role="button" aria-label="Ver productos de ${e.title}">
      <div class="ghost">${n(e.icon)}</div>
      <div class="p-ic">${n(e.icon)}</div>
      <span class="t-vert">${e.title}</span>
      <div class="p-body">
        <h3>${e.title}</h3>
        <p>${e.blurb}</p>
        <span class="p-go">Ver productos ${n("arrow")}</span>
      </div>
    </article>`}function i(e,t=!1){const a=c.find(r=>r.id===e);if(!a)return;document.querySelectorAll(".v-cat-panel").forEach(r=>r.classList.toggle("is-active",r.dataset.cat===e)),document.querySelectorAll(".v-chip").forEach(r=>r.classList.toggle("is-active",r.dataset.cat===e));const o=document.getElementById("v-products");o.innerHTML=`
    <div class="v-products-head"><div class="v-cat-ic">${n(a.icon)}</div><h2>${a.title}</h2></div>
    <div class="v-rail">${a.products.map(y).join("")}</div>`,o.classList.remove("revealing"),o.offsetWidth,o.classList.add("revealing"),t&&o.scrollIntoView({behavior:"smooth",block:"start"})}function k(){const e=document.getElementById("catalog");e.innerHTML=`
    <div class="v-deck">${c.map(w).join("")}</div>
    <div class="v-products" id="v-products"></div>`,e.querySelectorAll(".v-cat-panel").forEach(a=>{const o=a.dataset.cat;a.addEventListener("click",()=>i(o,!0)),a.addEventListener("keydown",r=>{(r.key==="Enter"||r.key===" ")&&(r.preventDefault(),i(o,!0))})});const t=document.getElementById("cat-chips");t.innerHTML=c.map(a=>`<button class="v-chip" data-cat="${a.id}">${a.title}</button>`).join(""),t.querySelectorAll(".v-chip").forEach(a=>a.addEventListener("click",()=>i(a.dataset.cat,!0))),i(c[0].id)}const l=[];function M(){const e=document.getElementById("promos");if(!l.length){e.style.display="none";return}e.innerHTML=`
    ...
    </div>`}function A(){const e=document.getElementById("quote-form"),t=document.getElementById("quote-status");e.addEventListener("submit",a=>{a.preventDefault();const o=new FormData(e),r=String(o.get("nombre")??"").trim(),m=String(o.get("whatsapp")??"").trim();if(!r||!m){t.textContent="Completá nombre y WhatsApp.",t.className="text-sm text-red-600";return}const p=`Cotización GNH%0ANombre: ${r}%0AEmpresa: ${o.get("empresa")??""}%0AProducto: ${o.get("producto")??""}%0AMensaje: ${o.get("mensaje")??""}`;window.open(`https://wa.me/${d}?text=${p}`,"_blank"),t.textContent="¡Gracias! Te redirigimos a WhatsApp.",t.className="text-sm text-green-600",e.reset()})}k();M();A();h();
