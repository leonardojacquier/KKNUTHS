import"./style-Cs5EtRwa.js";const c={instagram:"https://www.instagram.com/gnhimportacionespy/",facebook:"https://www.facebook.com/edzon.camilomazzonrtto",tiktok:"https://www.tiktok.com/@gnh",whatsapp:"https://wa.me/595985311031?text=Hola,%20quiero%20más%20información"},T=[{ciudad:"Ciudad del Este",dir:"Av. República del Perú km 7, CDE 100101",tel:"0995 360060",share:"https://share.google/OAtbh8akZyeNATtUM",q:"Av. República del Perú km 7, Ciudad del Este, Paraguay"},{ciudad:"Asunción · Ñemby",dir:"Acceso Sur, Ñemby 111210",tel:"",share:"https://share.google/DB7b1OL7m1ISorPhY",q:"Acceso Sur, Ñemby, Paraguay"}];function H(e="footer-slot"){const r=document.getElementById(e);if(!r)return;r.innerHTML=`
      ...
      <div class="mx-auto max-w-6xl px-6 py-12 grid gap-10 md:grid-cols-3 border-t border-white/10 mt-12">
        <div>
          ...
          <div class="mt-5 flex gap-3">
            <a href="${c.instagram}" target="_blank" rel="noopener" aria-label="Instagram" class="foot-social">${z}</a>
    ...
    </div>`;const t=Array.from(e.querySelectorAll(".vh-slide")),o=Array.from(e.querySelectorAll(".vh-seg")),n=Array.from(e.querySelectorAll(".vh-seg-fill")),s=e.querySelector(".vh-pause"),u=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let d=0,b=u;const x=()=>{t.forEach((i,$)=>{const C=$===d;i.classList.toggle("is-active",C);const v=i.querySelector("video");if(v)if(C){try{v.currentTime=0}catch{}v.play().catch(()=>{})}else v.pause()}),o.forEach((i,$)=>{i.classList.remove("is-active","is-done"),$<d&&i.classList.add("is-done")});const a=o[d];a.offsetWidth,a.classList.add("is-active")},f=a=>{d=(a+p.length)%p.length,x()},y=()=>f(d+1),w=()=>f(d-1);n.forEach(a=>a.addEventListener("animationend",()=>{!b&&o[d].classList.contains("is-active")&&y()}));const M=a=>{b=a,e.classList.toggle("is-paused",a),s.setAttribute("aria-pressed",String(a)),s.innerHTML=a?V:A};s.addEventListener("click",()=>M(!b)),e.querySelector(".vh-next").addEventListener("click",y),e.querySelector(".vh-prev").addEventListener("click",w),o.forEach(a=>a.addEventListener("click",()=>f(Number(a.dataset.i)))),e.querySelectorAll(".vh-cta").forEach(a=>a.addEventListener("click",()=>m(a.dataset.cat||"equipos",!0)));const k=e.querySelector(".vh-track");let g=null;k.addEventListener("touchstart",a=>{g=a.touches[0].clientX},{passive:!0}),k.addEventListener("touchend",a=>{if(g!==null){const i=a.changedTouches[0].clientX-g;Math.abs(i)>40&&(i<0?y():w())}g=null},{passive:!0}),x(),u&&M(!0)}function E(e){const r=`Hola, me interesa: ${e.name}${e.brand?" ("+e.brand+")":""}`;return`
    ...
    </div>`}function R(){const e=document.getElementById("quote-form"),r=document.getElementById("quote-status");e.addEventListener("submit",t=>{t.preventDefault();const o=new FormData(e),n=String(o.get("nombre")??"").trim(),s=String(o.get("whatsapp")??"").trim();if(!n||!s){r.textContent="Completá nombre y WhatsApp.",r.className="text-sm text-red-600";return}const u=`Cotización GNH%0ANombre: ${n}%0AEmpresa: ${o.get("empresa")??""}%0AProducto: ${o.get("producto")??""}%0AMensaje: ${o.get("mensaje")??""}`;window.open(`https://wa.me/${q}?text=${u}`,"_blank"),r.textContent="¡Gracias! Te redirigimos a WhatsApp.",r.className="text-sm text-green-600",e.reset()})}N();G();_();R();H();
