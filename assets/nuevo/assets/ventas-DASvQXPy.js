import"./style-CsVGQoPs.js";const n={instagram:"https://www.instagram.com/gnhimportacionespy/",facebook:"https://www.facebook.com/edzon.camilomazzonrtto",tiktok:"https://www.tiktok.com/@gnh",whatsapp:"https://wa.me/595985311031?text=Hola,%20quiero%20más%20información"},h=[{ciudad:"Ciudad del Este",dir:"Av. República del Perú km 7, CDE 100101",tel:"0995 360060",share:"https://share.google/OAtbh8akZyeNATtUM",q:"Av. República del Perú km 7, Ciudad del Este, Paraguay"},{ciudad:"Asunción · Ñemby",dir:"Acceso Sur, Ñemby 111210",tel:"",share:"https://share.google/DB7b1OL7m1ISorPhY",q:"Acceso Sur, Ñemby, Paraguay"}];function v(e="footer-slot"){const t=document.getElementById(e);if(!t)return;t.innerHTML=`
      ...
      <div class="mx-auto max-w-6xl px-6 py-12 grid gap-10 md:grid-cols-3 border-t border-white/10 mt-12">
        <div>
          ...
          <div class="mt-5 flex gap-3">
            ...
            <a href="${n.tiktok}" target="_blank" rel="noopener" aria-label="TikTok" class="foot-social">${y}</a>
          </div>
        </div>
        <div>
    ...
    <article class="v-card">
      ...
      <div class="v-card-body">
        ...
        <a class="v-cta" href="${g(t)}" target="_blank" rel="noopener" data-ev="product" data-detail="${e.name}">Consultar ${i("arrow","v-cta-i")}</a>
      </div>
    ...
    </article>`}function c(e,t=!1){const a=l.find(s=>s.id===e);if(!a)return;document.querySelectorAll(".v-cat-panel").forEach(s=>s.classList.toggle("is-active",s.dataset.cat===e)),document.querySelectorAll(".v-chip").forEach(s=>s.classList.toggle("is-active",s.dataset.cat===e));let o;a.groups&&a.groups.length?o=a.groups.map(s=>`
      <div class="v-subgroup">
        <h3 class="v-subtitle">${s.title}</h3>
        <div class="v-rail">${s.products.map(d).join("")}</div>
      </div>`).join(""):a.products&&a.products.length?o=`<div class="v-rail">${a.products.map(d).join("")}</div>`:o=`
      <div class="v-empty">
        <p>Pronto sumaremos productos de esta línea.</p>
        <a class="btn-empty" href="${g("Hola, quiero consultar sobre "+a.title)}" target="_blank" rel="noopener">Consultar por WhatsApp</a>
      </div>`;const r=document.getElementById("v-products");r.innerHTML=`
    <div class="v-products-head"><div class="v-cat-ic">${i(a.icon)}</div><h2>${a.title}</h2></div>
    ${o}`,r.classList.remove("revealing"),r.offsetWidth,r.classList.add("revealing"),t&&r.scrollIntoView({behavior:"smooth",block:"start"})}function k(){const e=document.getElementById("catalog");e.innerHTML=`
    <div class="v-deck">${l.map(w).join("")}</div>
    <div class="v-products" id="v-products"></div>`,e.querySelectorAll(".v-cat-panel").forEach(a=>{const o=a.dataset.cat;a.addEventListener("click",()=>c(o,!0)),a.addEventListener("keydown",r=>{(r.key==="Enter"||r.key===" ")&&(r.preventDefault(),c(o,!0))})});const t=document.getElementById("cat-chips");t.innerHTML=l.map(a=>`<button class="v-chip" data-cat="${a.id}">${a.title}</button>`).join(""),t.querySelectorAll(".v-chip").forEach(a=>a.addEventListener("click",()=>c(a.dataset.cat,!0))),c(l[0].id)}const p=[];function M(){const e=document.getElementById("promos");if(!p.length){e.style.display="none";return}e.innerHTML=`
    ...
    </div>`}function A(){const e=document.getElementById("quote-form"),t=document.getElementById("quote-status");e.addEventListener("submit",a=>{a.preventDefault();const o=new FormData(e),r=String(o.get("nombre")??"").trim(),s=String(o.get("whatsapp")??"").trim();if(!r||!s){t.textContent="Completá nombre y WhatsApp.",t.className="text-sm text-red-600";return}const u=`Cotización GNH%0ANombre: ${r}%0AEmpresa: ${o.get("empresa")??""}%0AProducto: ${o.get("producto")??""}%0AMensaje: ${o.get("mensaje")??""}`;window.open(`https://wa.me/${m}?text=${u}`,"_blank"),t.textContent="¡Gracias! Te redirigimos a WhatsApp.",t.className="text-sm text-green-600",e.reset()})}k();M();A();v();
