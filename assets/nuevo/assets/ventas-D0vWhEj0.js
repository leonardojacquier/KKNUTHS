import"./style-Cs5EtRwa.js";const c={instagram:"https://www.instagram.com/gnhimportacionespy/",facebook:"https://www.facebook.com/edzon.camilomazzonrtto",tiktok:"https://www.tiktok.com/@gnh",whatsapp:"https://wa.me/595985311031?text=Hola,%20quiero%20más%20información"},q=[{ciudad:"Ciudad del Este",dir:"Av. República del Perú km 7, CDE 100101",tel:"0995 360060",share:"https://share.google/OAtbh8akZyeNATtUM",q:"Av. República del Perú km 7, Ciudad del Este, Paraguay"},{ciudad:"Asunción · Ñemby",dir:"Acceso Sur, Ñemby 111210",tel:"",share:"https://share.google/DB7b1OL7m1ISorPhY",q:"Acceso Sur, Ñemby, Paraguay"}];function S(e="footer-slot"){const r=document.getElementById(e);if(!r)return;r.innerHTML=`
    ...
    <div class="vh-controls">
      <div class="vh-segs">${p.map((t,n)=>`<button class="vh-seg" data-i="${n}" aria-label="Ver ${n+1}"><span class="vh-seg-fill"></span></button>`).join("")}</div>
      <button class="vh-pause" aria-label="Pausar o reanudar" aria-pressed="false">${k}</button>
    </div>`;const a=Array.from(e.querySelectorAll(".vh-slide")),o=Array.from(e.querySelectorAll(".vh-seg")),i=Array.from(e.querySelectorAll(".vh-seg-fill")),s=e.querySelector(".vh-pause"),g=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let d=0,h=g;const $=()=>{a.forEach((n,y)=>n.classList.toggle("is-active",y===d)),o.forEach((n,y)=>{n.classList.remove("is-active","is-done"),y<d&&n.classList.add("is-done")});const t=o[d];t.offsetWidth,t.classList.add("is-active")},b=t=>{d=(t+p.length)%p.length,$()},f=()=>b(d+1),x=()=>b(d-1);i.forEach(t=>t.addEventListener("animationend",()=>{!h&&o[d].classList.contains("is-active")&&f()}));const w=t=>{h=t,e.classList.toggle("is-paused",t),s.setAttribute("aria-pressed",String(t)),s.innerHTML=t?P:k};s.addEventListener("click",()=>w(!h)),e.querySelector(".vh-next").addEventListener("click",f),e.querySelector(".vh-prev").addEventListener("click",x),o.forEach(t=>t.addEventListener("click",()=>b(Number(t.dataset.i)))),e.querySelectorAll(".vh-cta").forEach(t=>t.addEventListener("click",()=>m(t.dataset.cat||"equipos",!0)));const M=e.querySelector(".vh-track");let u=null;M.addEventListener("touchstart",t=>{u=t.touches[0].clientX},{passive:!0}),M.addEventListener("touchend",t=>{if(u!==null){const n=t.changedTouches[0].clientX-u;Math.abs(n)>40&&(n<0?f():x())}u=null},{passive:!0}),$(),g&&w(!0)}function C(e){const r=`Hola, me interesa: ${e.name}${e.brand?" ("+e.brand+")":""}`;return`
    ...
    <article class="v-cat-panel" data-cat="${e.id}" tabindex="0" role="button" aria-label="Ver productos de ${e.title}">
      ...
      </div>`;const i=document.getElementById("v-products");i.innerHTML=`
    <div class="v-products-head"><div class="v-cat-ic">${l(a.icon)}</div><h2>${a.title}</h2></div>
    ${o}`,i.classList.remove("revealing"),i.offsetWidth,i.classList.add("revealing"),r&&i.scrollIntoView({behavior:"smooth",block:"start"})}function N(){const e=document.getElementById("catalog");e.innerHTML=`
    <div class="v-deck">${v.map(V).join("")}</div>
    <div class="v-products" id="v-products"></div>`,e.querySelectorAll(".v-cat-panel").forEach(a=>{const o=a.dataset.cat;a.addEventListener("click",()=>m(o,!0)),a.addEventListener("keydown",i=>{(i.key==="Enter"||i.key===" ")&&(i.preventDefault(),m(o,!0))})});const r=document.getElementById("cat-chips");r&&(r.innerHTML=v.map(a=>`<button class="v-chip" data-cat="${a.id}">${a.title}</button>`).join(""),r.querySelectorAll(".v-chip").forEach(a=>a.addEventListener("click",()=>m(a.dataset.cat,!0)))),m(v[0].id)}const A=[];function D(){const e=document.getElementById("promos");if(!A.length){e.style.display="none";return}e.innerHTML=`
    ...
    </div>`}function G(){const e=document.getElementById("quote-form"),r=document.getElementById("quote-status");e.addEventListener("submit",a=>{a.preventDefault();const o=new FormData(e),i=String(o.get("nombre")??"").trim(),s=String(o.get("whatsapp")??"").trim();if(!i||!s){r.textContent="Completá nombre y WhatsApp.",r.className="text-sm text-red-600";return}const g=`Cotización GNH%0ANombre: ${i}%0AEmpresa: ${o.get("empresa")??""}%0AProducto: ${o.get("producto")??""}%0AMensaje: ${o.get("mensaje")??""}`;window.open(`https://wa.me/${E}?text=${g}`,"_blank"),r.textContent="¡Gracias! Te redirigimos a WhatsApp.",r.className="text-sm text-green-600",e.reset()})}j();N();D();G();S();
