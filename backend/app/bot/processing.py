...
from app.bot.link_do_clube import (_NOME_CLUBE, _REPLAY_HOSTS,
                                   replay_fallback_text,
                                   replay_link_info)
...
def estrategia_do_torneio(telegram_id: int, escolha: int = 1) -> str:
    ...
    # Por isso a fonte é outra — e o texto diz que é outra, senão o aluno lê
    # como se fosse do torneio que ele acabou de mandar.
    try:
        repo = get_repository()
        user = repo.get_or_create_user(telegram_id) if repo.enabled else None
        completas, _fora_da_amostra = (
            repo.get_hands_para_perfil(user["id"], limit=1500)
            if user else ([], 0))
        bloco = texto_da_frequencia(completas)
        if bloco:
            fora.append("\n\n— — —\n_As duas seções abaixo olham TODO o seu "
                        "histórico de sessões inteiras, não só este torneio._"
                        "\n\n" + bloco)
    except Exception as exc:
        log.warning("frequência por faixa falhou: %s", exc)
    return "\n".join(fora)
...
# reexportado: `handlers` importa daqui, e o módulo novo é folha
from app.bot.storyboard import storyboard_spot_from_drill  # noqa: F401,E402
...
# fluxo da foto do lobby: extraído para caber no teto de linhas deste módulo
# (o guarda test_processing_nao_incha disparou em 3607/3600). Reexportado
# porque handler, testes e o /preparar chamam por aqui.
from app.bot.lobby_flow import (  # noqa: E402
    CHAVE_ATUAL,
    _lobby_guardado,
    _slug_do_torneio,
    processar_lobby,
...
def villain_report(telegram_id: int, name: str) -> str:
    ...
    # e as apostas que NINGUÉM viu — sem "provavelmente blefou" (o showdown
    # seleciona quem paga; taxa dali seria enviesada). O que se mede com
    # exatidão é a defesa do aluno contra a aposta dele.
    from app.analysis.defesa import medir as medir_defesa
    from app.analysis.defesa import nao_vistas
    from app.analysis.defesa import texto as texto_da_defesa

    bloco = texto_da_defesa(medir_defesa(hands, prof["vilao"]),
                            nao_vistas(hands, prof["vilao"]), prof["vilao"])
    if bloco.strip():
        l.append(bloco)
